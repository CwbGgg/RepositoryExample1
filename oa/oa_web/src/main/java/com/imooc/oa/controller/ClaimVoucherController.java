...
public class ClaimVoucherController {
    ...
    public String toAdd(Map<String,Object> map){  //跳转到去添加的页面
    ...
    @RequestMapping("/add")
    public String add(HttpSession session, ClaimVoucherInfo info){ //info属性利用Springmvc的属性驱动自动装填信息
    ...
    public String detail(int id, Map<String,Object> map){
        map.put("claimVoucher",claimVoucherBiz.get(id));  //存入报修单的单号
        map.put("items",claimVoucherBiz.getItems(id)); //存入报修单中的多表信息记录
    ...
    public String self(HttpSession session, Map<String,Object> map){
        Employee employee = (Employee)session.getAttribute("employee");
        map.put("list",claimVoucherBiz.getForSelf(employee.getSn())); //通过报修单创建人的id(sn)来查询自己创建的报修单的信息记录
        return "claim_voucher_self";
    }


    @RequestMapping("/deal")
    public String deal(HttpSession session, Map<String,Object> map){//获得待处理的报修单
        Employee employee = (Employee)session.getAttribute("employee");
